...
import { useGetSalonQuery, useUpdateSalonMutation } from "../redux/api/salonApi";

const SalonEdit = () => {
    const navigate = useNavigate();

    const [isProcessing, setProcessing] = useState(false);
    const [updateSalon, { isLoading, isError, error, isSuccess }] = useUpdateSalonMutation();
    const { id } = useParams();
    const { data: salon } = useGetSalonQuery(id);
    ...
    useEffect(() => {
        if (salon) {
            ...
            fields.forEach((field) => setValue(field, salon[field]));
            setAddressObj(salon.location);
        }
    }, [setValue, salon, setAddressObj]);

    useEffect(() => {
        if (isSuccess) {
            toast.success('You successfully updated Salon!');
    ...
    return (
        <div className='main-board'>
            <Container>
                <Row>
                    <Col>
                        <h5 className="display-6">Edit Salon</h5>
                    </Col>
                </Row>
